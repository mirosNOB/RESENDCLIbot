...
@dp.message(StateFilter(None), ~F.text.in_(["➕ Добавить админа", "📋 Последние сообщения"]))
async def handle_general_messages(message: types.Message):
    """Обрабатывает обычные сообщения вне состояний."""
    user_id = message.from_user.id
    
    if message.text and message.text.startswith('/'):
        logger.info(f"Пользователь {user_id} отправил неизвестную команду: {message.text}")
        return  # Пропускаем команды, которые не обработаны другими обработчиками
